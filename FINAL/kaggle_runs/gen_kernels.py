#!/usr/bin/env python3
"""Generate per-(model, seed) Kaggle kernels for GRACE Track 1.

Faithful reproduction of notebook94c050a41f.ipynb's "Two-Stage Auto-Runner"
(cell 23) + cell 11 dev-benchmark additions, with per-model config overrides
so each model trains correctly instead of crashing/NaN-ing on a string swap.

Each kernel runs ONE (model, seed):
  Phase 1: train on train, eval on dev per-epoch (strict macro+per-class F1),
           EarlyStopping patience 4, find best epoch, save dev logits +
           dev predictions JSON (GRACE format) + per-epoch metrics CSV/JSON.
  Phase 2: full-fit on train+dev for (best_epoch+1), save blind-test logits
           per epoch (.npy) for ensembling.

Artifacts (all in /kaggle/working, committed via files-only persistence):
  {prefix}_seed{seed}_dev_logits.npy
  {prefix}_seed{seed}_dev_predictions.json
  {prefix}_seed{seed}_metrics.csv
  {prefix}_seed{seed}_metrics.json
  {prefix}_seed{seed}_epoch{N}_blind_logits.npy   (one per Phase-2 epoch)
  {prefix}_seed{seed}_log.txt                      (full stdout)
"""
...
KAGGLE_USERNAME = "prekzursil"
DATASET_SOURCE = "alexcristea72/iberlef-grace"
SEEDS = [42, 100, 2026]

# (model_id, file_prefix, slug_frag, use_fp16, extra_model_kwargs_suffix)
# extra_model_kwargs_suffix is appended verbatim inside from_pretrained(...).
# (model_id, file_prefix, short_slug, use_fp16, extra_model_kwargs, tf_spec)
# short_slug kept SHORT + fresh (original long slugs got wedged by failed pushes).
MODELS = [
    # ModernBERT -> needs eager attention (no flash-attn on Kaggle T4/P100)
    ("BSC-LT/MrBERT-biomed", "mrbert_biomed", "mrb", True,
     ', attn_implementation="eager"', "transformers>=4.48.0"),
    # RoBERTa-base (clinical EHR) -> vanilla
    ("PlanTL-GOB-ES/bsc-bio-ehr-es", "bsc_bio_ehr", "bsc", True, "", "transformers>=4.48.0"),
    # RoBERTa-base (biomedical-clinical) -> vanilla
    ("PlanTL-GOB-ES/roberta-base-biomedical-clinical-es", "roberta_clinical",
     "rob", True, "", "transformers>=4.48.0"),
    # DeBERTa-v3 -> fp16 OFF + pin transformers (latest NaNs grads on torch 2.10)
    ("microsoft/mdeberta-v3-base", "mdeberta_v3", "mdb", False, "", "transformers==4.46.3"),
    # XLM-R-large + Spanish clinical LAPT (~355M, 1024h)
    ("IIC/RigoBERTa-Clinical", "rigoberta_clinical", "rig", True, "", "transformers>=4.48.0"),
    # ModernBERT (Spanish) -> eager attention like MrBERT-biomed
    ("BSC-LT/MrBERT-es", "mrbert_es", "mre", True,
     ', attn_implementation="eager"', "transformers>=4.48.0"),
    # XLM-R-large (560M, 1024h) multilingual baseline
    ("FacebookAI/xlm-roberta-large", "xlmr_large", "xlr", True, "", "transformers>=4.48.0"),
    # BETO (Spanish BERT-base)
    ("dccuchile/bert-base-spanish-wwm-cased", "beto", "bet", True, "", "transformers>=4.48.0"),
    # BETO_Galen (BERT-base clinical Spanish; BertModel arch -> head added)
    ("IIC/BETO_Galen", "beto_galen", "gal", True, "", "transformers>=4.48.0"),
]

TEMPLATE = r'''# =====================================================================
# GRACE Track 1 - Two-Stage Auto-Runner  (model=@@MODEL_NAME@@, seed=@@SEED@@)
...
_pip("@@TF_SPEC@@", "sentencepiece", "accelerate")
...
def make_model():
    return AutoModelForSequenceClassification.from_pretrained(
        MODEL_NAME, num_labels=4, ignore_mismatched_sizes=True@@EXTRA_MODEL_KWARGS@@)
...
def main() -> None:
    ...
    manifest = []
    for model_id, prefix, short, use_fp16, extra_kwargs, tf_spec in MODELS:
        for seed in SEEDS:
            code = (TEMPLATE
                    .replace("@@SEED@@", str(seed))
                    .replace("@@MODEL_NAME@@", model_id)
                    .replace("@@FILE_PREFIX@@", prefix)
                    .replace("@@USE_FP16@@", "True" if use_fp16 else "False")
                    .replace("@@EXTRA_MODEL_KWARGS@@", extra_kwargs)
                    .replace("@@TF_SPEC@@", tf_spec))
            slug = f"gt1-{short}-s{seed}"          # short + fresh (old slugs wedged)
            kdir = root / slug
            kdir.mkdir(parents=True, exist_ok=True)
            code_file = f"{prefix}_seed{seed}.py"
            (kdir / code_file).write_text(code, encoding="utf-8")
            # init.py used for the reliable CPU "create" step (machine_shape/dataset
            # on a brand-new kernel can 404; create minimal, then update to full T4).
            (kdir / "init.py").write_text('print("init ok")\n', encoding="utf-8")
            meta = {
                "id": f"{KAGGLE_USERNAME}/{slug}",
                "title": f"gt1 {short} s{seed}",
                "code_file": code_file,
                "language": "python",
                "kernel_type": "script",
                "is_private": True,
                "enable_gpu": True,
                "machine_shape": "NvidiaTeslaT4",  # P100 sm_60 incompatible w/ torch 2.10
                "enable_internet": True,
                "dataset_sources": [DATASET_SOURCE],
            }
            (kdir / "kernel-metadata.json").write_text(
                json.dumps(meta, indent=2), encoding="utf-8")
            manifest.append({"slug": slug, "model": model_id, "seed": seed,
                             "prefix": prefix, "dir": str(kdir)})
    (root / "manifest.json").write_text(json.dumps(manifest, indent=2), encoding="utf-8")
    print(f"Generated {len(manifest)} kernels under {root}")
    for m in manifest:
        print(f"  {m['slug']:18s} {m['model']}  seed={m['seed']}")
